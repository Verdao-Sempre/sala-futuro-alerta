...
#  Versão GitHub Actions (roda na nuvem, sem computador)
# ============================================================
...
# Se não existir, usa os valores padrão (para testar no PC)
RA            = os.environ.get("RA",            "000110134488")
DIGITO        = os.environ.get("DIGITO",        "x")
SENHA         = os.environ.get("SENHA",         "mj13112008")
WHATSAPP      = os.environ.get("WHATSAPP",      "5511953054453")   # sem o +
CALLMEBOT_KEY = os.environ.get("CALLMEBOT_KEY", "")               # preenchido depois
...
def enviar_whatsapp(mensagem):
    """Envia mensagem via CallMeBot — funciona 100% na nuvem, sem browser."""
    if not CALLMEBOT_KEY:
        print("  ⚠️  CALLMEBOT_KEY não configurada. Mensagem não enviada.")
        print(f"  Mensagem que seria enviada:\n{mensagem}")
        return

    url = f"https://api.callmebot.com/whatsapp.php?phone={WHATSAPP}&text={quote(mensagem)}&apikey={CALLMEBOT_KEY}"
...
def main():
    ...
    print(f"\n{'='*50}")
    print(f"  Verificação iniciada: {agora}")
    print(f"{'='*50}")

    atividades_salvas = carregar_atividades_salvas()

    # Detectar se está rodando no GitHub Actions (Linux) ou no PC (Windows)
    is_cloud = os.environ.get("GITHUB_ACTIONS") == "true"

    try:
        with sync_playwright() as p:
            if is_cloud:
                # No GitHub Actions: headless obrigatório + flags Linux
                browser = p.chromium.launch(
                    headless=True,
                    args=["--no-sandbox", "--disable-setuid-sandbox", "--disable-dev-shm-usage"]
                )
            else:
                # No PC: abre janela visível para debug
        ...
        if novas:
            ...
            mensagem += f"Você tem {len(novas)} nova(s) atividade(s):\n\n"
            for a in novas:
                mensagem += f"• {a}\n"
        ...
        print(f"\n  ❌ Erro: {erro}")
        raise  # re-lança para o GitHub Actions marcar como falha
